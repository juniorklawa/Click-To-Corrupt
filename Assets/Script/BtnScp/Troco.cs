...
public class Troco : MonoBehaviour {
	...
	void Start () {
		...
		custo = PlayerPrefs.GetFloat ("custotroco", custo);
		level = PlayerPrefs.GetInt ("leveltroco", level);
	...
	public void OnBuy () {
		...
		if (click.money >= custo) {
			...
			PlayerPrefs.SetFloat ("custotroco", custo);
			PlayerPrefs.SetInt ("leveltroco", level);
